...
class fcn_resnet50(thelper.nn.utils.Module):
    ...
    def set_task(self, task):
        assert isinstance(task,
                          thelper.tasks.Segmentation), "invalid task (EfficientNet currently only supports classif)"
        ...
        if num_classes != self.num_classes:
            ...
            self.model.classifier[4] = torch.nn.Conv2d(in_channels=512, out_channels=num_classes, kernel_size=(1, 1),
                                                       stride=(1, 1))
            self.num_classes = num_classes


class fcn_resnet101(thelper.nn.utils.Module):
